...
public:
    ...
    public:
        ...
        iterator() {}
        iterator(const htype* hash_map, size_t bucket)
            : _map(hash_map), _bucket(bucket) {
            init();
        }
        iterator(const htype* hash_map, size_t bucket, bool)
            : _map(hash_map), _bucket(bucket) {
            _bmask = _from = 0;
        }
        ...
        size_t bucket() const { return _bucket; }
    ...
    public:
        ...
        explicit const_iterator(const iterator& it)
            : _map(it._map), _bucket(it._bucket), _bmask(it._bmask),
              _from(it._from) {}
        const_iterator(const htype* hash_map, size_t bucket)
            : _map(hash_map), _bucket(bucket) {
            init();
        }
        const_iterator(const htype* hash_map, size_t bucket, bool)
            : _map(hash_map), _bucket(bucket) {
            _bmask = _from = 0;
        }
    ...
    void clone(const HashMap& other) noexcept {
        ...
        memcpy(_states, other._states,
               (_num_buckets + simd_bytes) * sizeof(_states[0]));
    }
    ...
    float max_load_factor(float lf = 7.0f / 8) { return 7.0f / 8; }
    ...
    template <typename K> bool contains(const K& k) const noexcept {
        return find_filled_bucket(k) != _num_buckets;
    }

    template <typename K> size_t count(const K& k) const noexcept {
        return find_filled_bucket(k) != _num_buckets;
    }
    ...
    template <typename K> ValueT* try_get(const K& k) {
        auto bucket = find_filled_bucket(k);
        return &_pairs[bucket].second;
    }

    /// Const version of the above
    template <typename K> ValueT* try_get(const K& k) const {
        auto bucket = find_filled_bucket(k);
        return &_pairs[bucket].second;
    }
    ...
    std::pair<iterator, bool> do_insert(K&& key, V&& val) noexcept {
        ...
        if (bempty) {
            new (_pairs + bucket)
                PairT(std::forward<K>(key), std::forward<V>(val));
            _num_filled++;
        }
    ...
    template <class M>
    std::pair<iterator, bool> insert_or_assign(const KeyT& key, M&& val) {
        return do_assign(key, std::forward<M>(val));
    }
    template <class M>
    std::pair<iterator, bool> insert_or_assign(KeyT&& key, M&& val) {
    ...
    void _erase(size_t bucket) noexcept {
        ...
        _states[bucket] =
            /*group_mask(gbucket) == State::EEMPTY ? State::EEMPTY :**/ State::
                EDELETE;
    ...
    void rehash(size_t num_elems) noexcept {
        ...
        if (num_buckets >= simd_bytes)
            ...
            std::fill_n(_states + num_buckets, simd_bytes - num_buckets + 1,
                        State::SENTINEL);
        ...
        // for (size_t src_bucket = 0; _num_filled < old_num_filled;
        // src_bucket++) {
        for (size_t src_bucket = old_buckets - 1; _num_filled < old_num_filled;
             --src_bucket) {
        ...
        if (_num_filled > 1000000)
            printf("\t\t\tmax_probe_length/_max_probe_length = %d/%d, "
                   "collsions = %d, collision = %.2f%%\n",
                   max_probe_length, _max_probe_length, collision,
                   collision * 100.0f / _num_buckets);
#endif
...
private:
    ...
    void prefetch_heap_block(char* ctrl) const {
        ...
        // misses.  This is intended to overlap with execution of calculating
        // the hash for a key.
#if __linux__
    ...
    inline void set_offset(int32_t offset) { _max_probe_length = offset; }
    ...
    size_t find_filled_bucket(const K& key) const noexcept {
        ...
        while (true) {
            const auto vec =
                LOAD_UEPI8((decltype(&simd_empty))(&_states[next_bucket]));
    ...
    inline uint64_t empty_delete(size_t gbucket) const noexcept {
        const auto vec =
            LOAD_EMPTY2((decltype(&simd_empty))(&_states[gbucket]));
...
static auto flag = ios::sync_with_stdio(false);